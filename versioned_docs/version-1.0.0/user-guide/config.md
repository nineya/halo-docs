...
很多人反映评论中自己的头像不显示，其实评论部分调用的头像并不是在个人资料中设置的那个，而是 `Gravatar`。所以这一点怪我，从 Halo-Plus 发布至今我都没有说明这个事情。
...
A：先检查配置是否有误，如不知道哪里错误，请查看日志。点击左上角 `Halo-Plus Dashboard`即可进入开发者选项，点击 `实时日志` 即可看到最近的日志。
