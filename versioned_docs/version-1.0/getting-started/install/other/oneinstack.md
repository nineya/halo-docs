...
description: 使用 OneinStack 的 vhost 脚本创建 Halo-Plus 站点的 Nginx 配置文件
---
...
:::info
`「反向代理」` 部分不进行操作，保证 Halo-Plus 服务运行无误即可。
:::
...
> 即创建一个站点，你可以通过这样的方式在你的服务器创建无限个站点。接下来的目的就是创建一个站点，并反向代理到 Halo-Plus。这一步在此教程使用 `demo.halo-plus.nineya.com` 这个域名做演示，实际情况请修改此域名。
...
```bash
Please input the directory for the domain:demo.halo-plus.nineya.com :
...
```bash
Your domain:                  demo.halo-plus.nineya.com
...
Nginx 的配置文件即 `/usr/local/nginx/conf/vhost/demo.halo-plus.nineya.com.conf`。
...
```bash
vim /usr/local/nginx/conf/vhost/demo.halo-plus.nineya.com.conf
```
...
如果不按照第 5，6 步操作，请求一些图片或者样式文件不会经过 Halo-Plus，所以请不要忽略此配置。
